                          ...
                          <div className="absolute left-0 mt-2 w-80 bg-white rounded-xl shadow-2xl py-2 text-gray-800 z-50 overflow-hidden border border-gray-100 animate-slide-up">
                              ...
                              <div className="max-h-80 overflow-y-auto scrollbar-thin">
                                  {notifications.length === 0 ? (
                                      <div className="p-8 text-center text-gray-400 text-sm">لا توجد إشعارات حالياً</div>
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-sea-50">
      <Card className="max-w-md w-full p-8 space-y-8 animate-slide-up border-t-4 border-t-sea-500">
        <div><h2 className="mt-2 text-center text-3xl font-extrabold text-sea-900">تسجيل الدخول</h2></div>
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <div className="text-sm text-center"><Link to="/register" className="font-medium text-sea-600 hover:text-sea-500 transition-colors">ليس لديك حساب؟ <span className="underline">سجل الآن</span></Link></div>
        </form>
      </Card>
    </div>
  );
...
const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.role === 'provider') {
        const newProvider: ProviderProfile = { providerId: userCredential.user.uid, name: formData.name, description: "طباخ ماهر يقدم أشهى المأكولات البحرية", followersCount: 0 }
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-sea-50">
      <Card className="max-w-md w-full p-8 space-y-8 animate-slide-up border-t-4 border-t-sea-500">
        <div><h2 className="mt-2 text-center text-3xl font-extrabold text-sea-900">حساب جديد</h2></div>
        <form className="mt-8 space-y-4" onSubmit={handleRegister}>
          ...
          <div className="text-sm text-center"><Link to="/login" className="font-medium text-sea-600 hover:text-sea-500">لديك حساب؟ سجل دخول</Link></div>
        </form>
      </Card>
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <div>
          ...
          <p className="text-gray-500 mt-2">أهلاً بك، إليك نظرة عامة على نشاطك اليوم</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="الأصناف في الكتالوج" value={stats.items} color="#3b82f6" icon={<ShoppingBagIcon className="w-6 h-6"/>} />
        <StatCard title="العروض النشطة" value={stats.offers} color="#10b981" icon={<CalendarIcon className="w-6 h-6"/>} />
        <StatCard title="إجمالي الطلبات" value={stats.reservations} color="#f59e0b" icon={<ClipboardDocumentCheckIcon className="w-6 h-6"/>} />
        <StatCard title="إجمالي الإيرادات" value={`${stats.revenue.toLocaleString()}`} color="#8b5cf6" icon={<CurrencyDollarIcon className="w-6 h-6"/>} />
      </div>
    </div>
  );
};
...
const ProviderReservations = () => {
    ...
    const handleBulkAction = async () => {
        ...
        selectedIds.forEach(id => {
            ...
            if(res) {
                ...
                sendNotification(res.customerId, 'تحديث الطلب', `تم تغيير حالة طلبك ${res.offeringName} إلى ${newStatus === 'confirmed' ? 'مؤكد' : 'مكتمل'}`);
            }
        });
        try {
            await batch.commit();
            showToast("تم تحديث الطلبات بنجاح", "success");
            setSelectedIds(new Set());
            fetchReservations();
        } catch(e) {
            showToast("حدث خطأ في التحديث الجماعي", "error");
        }
    };
    ...
    const handleSingleStatus = async (id: string, status: ReservationStatus) => {
        try {
            ...
            if(res) {
                 let msg = '';
                 if (status === 'confirmed') msg = 'تم تأكيد طلبك، جاري التحضير!';
                 else if (status === 'completed') msg = 'تم إكمال طلبك، شكراً لتعاملك معنا.';
                 else if (status === 'cancelled') msg = 'عذراً، تم إلغاء طلبك.';
                 
                 await sendNotification(res.customerId, 'تحديث حالة الطلب', `${msg} (${res.offeringName})`, status === 'cancelled' ? 'warning' : 'success');
            }
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-2">
                        <ClipboardDocumentCheckIcon className="w-8 h-8 text-sea-600"/> إدارة الحجوزات
                    </h1>
                </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white p-4 rounded-xl shadow-sm border-r-4 border-blue-500">
                        <p className="text-xs text-gray-500 font-bold mb-1">الطلبات</p>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm border-r-4 border-green-500">
                        <p className="text-xs text-gray-500 font-bold mb-1">المؤكدة</p>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm border-r-4 border-purple-500">
                        ...
                        <p className="text-2xl font-bold text-purple-600">{stats.revenue.toLocaleString()} <span className="text-xs">ر.ي</span></p>
                    </div>
                </div>
                ...
                <Card className="p-4 flex flex-col lg:flex-row gap-4 justify-between items-center sticky top-20 z-40 shadow-md">
                    <div className="flex flex-1 w-full gap-4 flex-col sm:flex-row">
                        <div className="relative flex-1">
                            ...
                            <input type="text" placeholder="بحث باسم العميل أو الصنف..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pr-10 pl-4 py-2 border rounded-lg focus:ring-2 focus:ring-sea-500 outline-none" />
                        </div>
                        <div className="flex bg-gray-100 p-1 rounded-lg overflow-x-auto">
                            {['all', 'pending', 'confirmed', 'completed', 'cancelled'].map(st => (
                                <button key={st} onClick={() => { setStatusFilter(st); setSelectedIds(new Set()); }} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all whitespace-nowrap ${statusFilter === st ? 'bg-white shadow text-sea-700' : 'text-gray-500 hover:text-gray-700'}`}>{st === 'all' ? 'الكل' : st === 'pending' ? 'معلق' : st === 'confirmed' ? 'مؤكد' : st === 'completed' ? 'مكتمل' : 'ملغي'}</button>
                            ))}
                        </div>
                    </div>
                ...
                {selectedIds.size > 0 && statusFilter !== 'all' && (
                    <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-6 py-3 rounded-full shadow-2xl z-50 flex items-center gap-4 animate-slide-up">
                        ...
                        <button onClick={handleBulkAction} className="hover:text-green-400 font-bold text-sm flex items-center gap-1">{statusFilter === 'pending' && <><CheckCircleIcon className="w-5 h-5"/> تأكيد الكل</>}{statusFilter === 'confirmed' && <><CheckBadgeIcon className="w-5 h-5"/> إكمال الكل</>}</button>
                ...
                {filteredReservations.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-dashed border-gray-300">
                        ...
                        <h3 className="text-xl font-bold text-gray-700">لا توجد طلبات مطابقة</h3>
                    </div>
                ) : (
                    <>
                    {groupBy === 'none' && (
                            ...
                            {groupedReservations.map((group, idx) => (
                                <div key={idx} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
                                    <div className="bg-gray-50 p-4 border-b flex justify-between items-center">
                                        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-sea-100 text-sea-700 flex items-center justify-center font-bold text-lg">{group.name[0]}</div><div><h3 className="font-bold text-gray-800">{group.name}</h3><p className="text-xs text-gray-500">{group.items.length} طلبات</p></div></div>
                                        <div className="text-right"><div className="text-lg font-bold text-sea-700">{group.total} ر.ي</div><div className="text-xs text-gray-400">الإجمالي</div></div>
                                    </div>
                                        ...
                                        {group.items.map(res => (
                                            <div key={res.id} className="p-4 hover:bg-gray-50 flex justify-between items-center">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div><p className="font-medium text-sm text-gray-800">{groupBy === 'item' ? res.customerName : res.offeringName} (x{res.quantity})</p><p className="text-xs text-gray-500">{new Date(res.createdAt).toLocaleTimeString('ar-SA')}</p>{res.paymentReference && <span className="text-[10px] text-green-600 font-bold block">سند: {res.paymentReference}</span>}</div>
                                                </div>
...
const MyReservations = () => {
    ...
    const cancelReservation = async (id: string, providerId: string, offeringName: string) => {
        ...
        try {
            ...
            await sendNotification(providerId, 'إلغاء طلب', `قام العميل بإلغاء طلب ${offeringName}`, 'warning');
        ...
        <div className="p-4 md:p-6 max-w-4xl mx-auto min-h-screen">
             ...
             <Card className="p-4 flex flex-col lg:flex-row gap-4 justify-between items-center mb-6 shadow-md">
                <div className="flex flex-1 w-full gap-4 flex-col sm:flex-row">
                    <div className="relative flex-1">
                        <MagnifyingGlassIcon className="absolute right-3 top-3 w-5 h-5 text-gray-400" />
                        <input type="text" placeholder="بحث في طلباتي..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pr-10 pl-4 py-2 border rounded-lg focus:ring-2 focus:ring-sea-500 outline-none" />
                    </div>
                    <div className="flex bg-gray-100 p-1 rounded-lg overflow-x-auto">
                        {['all', 'pending', 'confirmed', 'completed', 'cancelled'].map(st => (
                            <button key={st} onClick={() => setStatusFilter(st)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all whitespace-nowrap ${statusFilter === st ? 'bg-white shadow text-sea-700' : 'text-gray-500 hover:text-gray-700'}`}>{st === 'all' ? 'الكل' : st === 'pending' ? 'معلق' : st === 'confirmed' ? 'مؤكد' : st === 'completed' ? 'مكتمل' : 'ملغي'}</button>
                        ))}
                    </div>
                </div>
             </Card>
            ...
            <div className="space-y-4">
                {filteredReservations.length === 0 ? (
                     <div className="text-center py-12 bg-white rounded-lg shadow-sm">
                        ...
                        <p className="text-gray-500">لا توجد طلبات في هذا التاريخ</p>
                    </div>
                ) : filteredReservations.map(res => (
                    <Card key={res.id} className="p-6">
                        <div className="flex flex-col md:flex-row justify-between gap-6">
                                    ...
                                    <div className="bg-orange-50 p-3 rounded-lg border border-orange-100 mt-2">
                                        ...
                                        {res.paymentReference ? (
                                            ...
                                            <div className="flex gap-2">
                                                <input type="text" placeholder="رقم السند / عمقي / الكريمي" className="flex-1 border p-2 rounded text-sm outline-none focus:border-orange-400" value={paymentRefs[res.id] || ''} onChange={e => setPaymentRefs({...paymentRefs, [res.id]: e.target.value})} />
                            ...
                            <div className="flex flex-col justify-center items-end border-t md:border-t-0 md:border-r md:pr-4 pt-4 md:pt-0 border-gray-100">
                                {res.status === 'pending' && (
                                    <button onClick={() => cancelReservation(res.id, res.providerId, res.offeringName)} className="text-red-500 text-sm hover:underline hover:text-red-700 font-medium">إلغاء الطلب</button>
                                )}
                                ...
                                {res.status === 'completed' && (
                                    <div className="flex items-center gap-1 text-green-600 font-bold text-sm"><CheckBadgeIcon className="w-5 h-5"/> تم الاستلام</div>
                                )}
                            </div>
                        </div>
                    </Card>
...
const CustomerHome = () => {
    ...
    const handleReserve = async (offer: Offering) => {
        ...
        try {
            ...
            // Notify Provider
            await sendNotification(offer.providerId, 'طلب جديد', `قام ${userProfile?.name} بطلب وجبة ${offer.itemName}`, 'success');
             ...
             <div className="bg-gradient-to-r from-sea-800 to-sea-600 rounded-2xl p-8 mb-10 text-white shadow-xl flex flex-col md:flex-row items-center justify-between animate-fade-in relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <div className="bg-white/10 p-2 rounded-xl inline-flex items-center backdrop-blur-md border border-white/20 w-full max-w-md shadow-lg transition-all focus-within:bg-white/20">
                        ...
                        <input type="text" placeholder="ابحث عن وجبة، سمك، جمبري..." className="bg-transparent border-none text-white placeholder-sea-300 focus:ring-0 outline-none w-full font-medium" value={filter} onChange={(e) => setFilter(e.target.value)} />
                    </div>
                </div>
                        ...
                        <Card key={offer.id} className="group hover:-translate-y-2 transition-all duration-300 flex flex-col h-full border-gray-100 hover:shadow-xl hover:border-sea-200">
                            ...
                            <div className="p-5 flex flex-col flex-1">
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-gray-500 flex items-center gap-1 mb-4 bg-gray-50 p-2 rounded-lg"><UserIcon className="w-3 h-3"/> مقدم الخدمة: {offer.providerId.substring(0,8)}...</p>
                                </div>
                                <Button onClick={() => handleReserve(offer)} className="w-full mt-2 group-hover:bg-sea-700 transition-colors" disabled={offer.quantityRemaining <= 0}>{offer.quantityRemaining > 0 ? 'حجز الطلب الآن' : 'نفذت الكمية'}</Button>
                            </div>
                        </Card>
                    ))}
                </div>
